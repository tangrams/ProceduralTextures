...
vec2 u_windowSize = vec2(0.5,0.8);

float u_frequency = 4.0;

vec3 brick(vec2 _uv, float _zoom, vec3 _brickColor, vec3 _mortarColor){
	vec2 pos = _uv/((vec2(2.15,0.65))/(3.14*_zoom));
	if (fract(pos.y * 0.5) > 0.5){
		pos.x += 0.5;
	}
	pos = fract(pos);
	vec2 isBrick = step(pos,vec2(0.96,0.88));
...
void main(){
	...
	if (windowsMask.x * windowsMask.y <= 0.5) {
		color.rgb = brick(st,1.8*u_frequency,vec3(0.5),vec3(0.0));
	} else {
		color.rgb = vec3(1.0);
	}
